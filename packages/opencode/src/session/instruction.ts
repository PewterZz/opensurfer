...
const FILES = [
  "AGENTS.md",
  ...(Flag.OPENSURFER_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.OPENSURFER_CONFIG_DIR) {
    files.push(path.join(Flag.OPENSURFER_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.OPENSURFER_DISABLE_CLAUDE_CODE_PROMPT) {
...
export namespace Instruction {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@opensurfer/Instruction") {}
    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const relative = Effect.fnUntraced(function* (instruction: string) {
          ...
          if (!Flag.OPENSURFER_CONFIG_DIR) {
            log.warn(
              `Skipping relative instruction "${instruction}" - no OPENSURFER_CONFIG_DIR set while project config is disabled`,
            )
            return []
          }
          return yield* fs
            .globUp(instruction, Flag.OPENSURFER_CONFIG_DIR, Flag.OPENSURFER_CONFIG_DIR)
            .pipe(Effect.catch(() => Effect.succeed([] as string[])))
        })
