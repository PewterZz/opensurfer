...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".opensurfer"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".opensurfer", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): Config.PluginOrigin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".opensurfer", "tui.json"),
  }
}
  ...
  async function load(api: Api) {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const records = Flag.OPENSURFER_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.OPENSURFER_PURE && config.plugin_origins?.length) {
