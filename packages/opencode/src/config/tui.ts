...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.OPENSURFER_TUI_CONFIG
  }
  ...
  const state = Instance.state(async () => {
    let projectFiles = Flag.OPENSURFER_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.OPENSURFER_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".opensurfer") && dir !== Flag.OPENSURFER_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old opensurfer.json shape) still get their settings applied.
