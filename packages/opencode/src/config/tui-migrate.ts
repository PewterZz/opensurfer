...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from opensurfer.json files
...
async function opencodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.OPENSURFER_DISABLE_PROJECT_CONFIG
    ? []
  ...
  if (Flag.OPENSURFER_CONFIG) files.push(Flag.OPENSURFER_CONFIG)
