...
export namespace Plugin {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@opensurfer/Plugin") {}
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Plugin.state")(function* (ctx) {
          ...
          const client = createOpencodeClient({
            ...
            headers: Flag.OPENSURFER_SERVER_PASSWORD
              ? {
                  Authorization: `Basic ${Buffer.from(`${Flag.OPENSURFER_SERVER_USERNAME ?? "opensurfer"}:${Flag.OPENSURFER_SERVER_PASSWORD}`).toString("base64")}`,
                }
              : undefined,
          ...
          const plugins = Flag.OPENSURFER_PURE ? [] : (cfg.plugin_origins ?? [])
          if (Flag.OPENSURFER_PURE && cfg.plugin_origins?.length) {
